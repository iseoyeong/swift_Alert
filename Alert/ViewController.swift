...
class ViewController: UIViewController {
    ...
    @IBAction func btnLampOn(_ sender: UIButton) {
        if(isLampOn==true) {
//       대화상자 자체를 제어하는 역할, prefferredStyle(상수로 정의되어 있음)
            let lampOnAlert = UIAlertController(title: "경고", message: "현재 On 상태입니다", preferredStyle: UIAlertController.Style.alert)
//       nil 현재 버튼을 눌렀을 때 동작하는 부분이 비어있음. (동작 없이 닫을 때)
            let onAction = UIAlertAction(title: "네, 알겠습니다.", style: UIAlertAction.Style.default, handler: nil)
            
            lampOnAlert.addAction(onAction)
            present(lampOnAlert, animated: true, completion: nil)    //lamOnAlert라는 컨트롤러를 넣어줘서 표시
        } else {
            //전구 꺼져있는 경우 켜기
            lamping.image = imgOn
            isLampOn = true
    ...
    @IBAction func btnLampOff(_ sender: UIButton) {
        if isLampOn {
            ...
            let offAction = UIAlertAction(title: "네", style: UIAlertAction.Style.default, handler: {
                ACTION in self.lamping.image = self.imgOff; self.isLampOn=false
            }) //익명함수: 이름이 없는 함수. 사용 이유 : 함수를 여러 군데에서 사용 x,

            let cancelAction = UIAlertAction(title: "아니오", style: UIAlertAction.Style.default, handler: nil)
            
            //아래 집어 넣는 순서에 따라 버튼 순서 결정됨.
    ...
    @IBAction func btnLampRemove(_ sender: UIButton) {
        ...
        let onAction = UIAlertAction(title: "아니오, 켭니다(on)", style: UIAlertAction.Style.default, handler: {
            ACTION in self.lamping.image = self.imgOn; self.isLampOn=true
        })
        
        let removeAction = UIAlertAction(title: "네, 제거합니다.", style: UIAlertAction.Style.destructive, handler: {
            ACTION in self.lamping.image = self.imgRemove; self.isLampOn=false
        })
